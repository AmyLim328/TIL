...
/* 1번 2번 가능할까?
 * 오라클 설치 지역, 기본 언어 (날찌 표기 방법 => 한국식만 가능)
 */
...
INSERT INTO EMP_TEMP (EMPNO, ENAME, JOB, HIREDATE)
VALUES(8888, '홍길목', 'SALESMAN', SYSDATE);
...
-- Q1. SMITH보다 SAL 많이 받는 사원 정보 추가
SELECT * FROM TAB;

DROP TABLE EMP_TEMP_Q1;

CREATE TABLE EMP_TEMP_Q1 AS SELECT * FROM EMP WHERE 1<>1;
...
-- Q2. 30번 부서 평균 SAL보다 적게 받는 사원 정보 추가
DROP TABLE EMP_TEMP_Q2;

CREATE TABLE EMP_TEMP_Q2 AS SELECT * FROM EMP WHERE 1<>1;
...
-- Q2-1. 30번 부서 사람들 중에 평균 급여보다 적게 받는 사원 정보 추가
DROP TABLE EMP_TEMP_Q2_1;

CREATE TABLE EMP_TEMP_Q2_1 AS SELECT * FROM EMP WHERE 1<>1;
...
-- Q3. KING보다 입사일이 빠른 사원 정보 추가
DROP TABLE EMP_TEMP_Q3;

CREATE TABLE EMP_TEMP_Q3 AS SELECT * FROM EMP WHERE 1<>1;
...
-- Q4. 매니저 이름이 'BLAKE'인 사원 정보 추가
DROP TABLE EMP_TEMP_Q4;

CREATE TABLE EMP_TEMP_Q4 AS SELECT * FROM EMP WHERE 1<>1;
...
-- Q5. 급여 등급이 1등급인 사원 정보 추가
DROP TABLE EMP_TEMP_Q5;

CREATE TABLE EMP_TEMP_Q5 AS SELECT * FROM EMP WHERE 1<>1;

-- 조인 테이블에서 데이터 가져와서 테이블 추가
...
-- 조인 하지 않은 테이블에서 데이터 가져와서 테이블 추가 (서브쿼리 이용)
INSERT INTO EMP_TEMP_Q5
SELECT * FROM EMP
...
-- Q2. 급여 등급 3등급 (SALGRADE, EMP_TEMP2)인 사원 중에 30번 부서 사원 삭제
-- 기본키 통해서 삭제
DELETE FROM EMP_TEMP2
