...
-- 테이블이 여러 개로 쪼개져있는 특징 (테이블 간의 '관계' (외래키가 기본키가 되어서 검색!)를 지정함)
-- why?
...
-- SELECT * 
-- => 조인이 될 경우에 출력하고자 하는 열 이름을 다 작성하는 게 좋음
-- WHY?
-- 1) 동일한 열 이름이 있을 경우 오류 방지
-- 2) SELECT 뒤에 문장만 보고 어떤 테이블에서 해당 열이 출력되는지 확인하기 좋음
...
-- Q1.사원 번호, 이름, 급여 (EMP 테이블), 근무 부서이름 (DEPT 테이블)을 함께 출력
--    급여가 3000 이상인 데이터만 출력
SELECT E.EMPNO, E.ENAME, E.SAL, E.DEPTNO, D.DEPTNO, D.DNAME
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND E.SAL >= 3000;

-- Q2. 사원 번호, 이름, 급여 (EMP 테이블), 근무 부서이름 (DEPT 테이블)을 함께 출력
--    사원 번호의 앞 두 자리가 79로 시작 출력

SELECT E.EMPNO, E.ENAME, E.SAL, E.DEPTNO, D.DEPTNO, D.DNAME
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND SUBSTR(E.EMPNO, 1, 2) = '79';

-- Q3. 각 부서별 부서 번호, 부서 이름, (열 이름)
-- 다중행 함수
-- 평균 급여 (소수점 2번째 자리까지 표현(반올림))
-- 최대 급여, 최소 급여, 사원 수를 출력
-- EMP, DEPT 테이블
...
-- (SQL-99 이전 방식과 SQL-99 방식을 각각 사용하여 작성)
-- 	Q1 ~ Q4 교재에 있는 문제
...
-- Q2. 오른쪽과 같이 
-- 각 부서별 평균 급여, 최대 급여, 최소 급여, 사원 수를 출력
-- 부서의 부서 번호, 부서 이름 (DEPT)
-- 급여 (EMP)
-- GROUP BY DEPTNO, DNAME

-- 1) 비표준 방식
SELECT D.DEPTNO, D.DNAME, -- GROUP 기준이라 SELECT문에 작성 가능
...
-- 2) 비표준 방식
SELECT E.EMPNO, E.ENAME, D.DEPTNO, E.SAL, E.MGR
FROM EMP E, DEPT D
